...
static BOOL	is_need_to_draw(t_sprite sprite, int i, double *buffer, int x)
{
	return (sprite.transform.y > 0 && i > 0 &&
			i < x && sprite.transform.y < buffer[i]);
}
...
void		render_sprite(t_sprite sprite, t_game *game, double *buffer)
{
	...
	while (++i < sprite.draw_end.x)
	{
		...
		if (is_need_to_draw(sprite, i, buffer, game->config.res.x))
		{
			j = sprite.draw_start.y - 1;
			while (++j < sprite.draw_end.y)
			{
				tex_pos.y = (((j * 256 - game->config.res.y * 128 +
	sprite.height * 128) * game->sprites.texture.width) / sprite.height) / 256;
